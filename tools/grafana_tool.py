...
class GrafanaTool:
    ...
    def fetch_logs(self, service_name: str, duration_minutes: int = 15, limit: int = 50) -> str:
        """Fetch recent Loki logs for a service. Returns empty string if Grafana is not configured."""
    ...
    def _filter_logs(self, lines: list[str]) -> list[str]:
        """Keep ERROR/WARN/EXCEPTION lines; falls back to all lines if none match."""
